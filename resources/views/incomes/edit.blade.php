            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{route('incomes.update',$income)}}">
                    ...
                    <div>
                        ...
                        <x-text-input id="description" name="description" type="text" value="{{old('description',$income->description)}}" required/>
                        <x-input-error :messages="$errors->get('description')"/>
                    </div>

                    <div>
                        <x-input-label for="amount" value="Valor"/>
                        <x-text-input id="amount" name="amount" type="number" step="0.01" value="{{old('amount', $income->amount)}}" required/>
                    ...
                    <div>
                        ...
                        <x-text-input id="date" name="date" type="date" value="{{old('date', $income->date)}}" required/>
                        <x-input-error :messages="$errors->get('date')"/>
                    </div>

                    <div>
                        <x-input-label for="notes" value="Notas"/>
                        <textarea id="notes" name="notes" >{{ old('notes',$income->notes) }}</textarea>
